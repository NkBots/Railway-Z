    ...
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() != MirrorStatus.STATUS_CONVERTING:
                msg += f"\n<b>✤ For Cancel:</b> <code>/{BotCommands.CancelMirror} {download.gid()}</code>"
            msg += "\n\n"
        ...
        for download in list(download_dict.values()):
            if download.status() == MirrorStatus.STATUS_DOWNLOADING:
                ...
                if 'K' in spd:
                    ...
                    up_speed += float(spd.split('M')[0]) * 1048576
        bmsg = f"<b>Fʀᴇᴇ:</b> <code>{get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}</code><b> | Uᴘᴛɪᴍᴇ:</b> <code>{get_readable_time(time() - botStartTime)}</code>"
        bmsg += f"\n<b>⇓ Dʟ:</b> <code>{get_readable_file_size(dl_speed)}/s</code><b> | ⇑ UL:</b> <code>{get_readable_file_size(up_speed)}/s</code>"
        buttons = ButtonMaker()
        buttons.sbutton("💔 𝐀𝐍𝐆𝐄𝐋 𝐗 𝐎𝐩 𝐋𝐎𝐋 🦋", str(THREE))
