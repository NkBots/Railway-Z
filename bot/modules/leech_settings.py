...
def getleechinfo(from_user):
    ...
    if user_id in AS_DOC_USERS or user_id not in AS_MEDIA_USERS and AS_DOCUMENT:
        ltype = "DOCUMENT"
        buttons.sbutton("✔️Send As Media", f"leechset {user_id} med")
    else:
        ltype = "MEDIA"
        buttons.sbutton("✔️Send As Document", f"leechset {user_id} doc")
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        msg = f"✔️Hey SeD thumbnail saved for {update.message.from_user.mention_html(update.message.from_user.first_name)}."
        sendMessage(msg, context.bot, update.message)
    else:
        sendMessage("✔️SeD Reply to a photo to save custom thumbnail.", context.bot, update.message)
